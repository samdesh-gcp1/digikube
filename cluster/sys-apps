...
if [[ -z ${is_app_enabled} ]]; then
    ...
    if [[ "${is_app_enabled}" == "true" ]]; then
        ...
        sys_app_om_shell="${digi_dir}cluster/sys-apps-om/${sys_app_name}-om.sh"
        is_command_enabled=$(get-config-value "cluster.sysApps.${sys_app_name}.om-commands.${sys_app_command}")
